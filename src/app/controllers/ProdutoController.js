...
class ProdutoController {
    ...
    async search(req, res) {
        ...
        COALESCE((1-p1.preco_promocao / e.preco_venda)*100, 0) AS discount, e.qtd_estoque, 0 as qtd, e.id as id_estoque, e.fabricante, p.id_principio, p.classe_terapeutica
              FROM produtos p  
               INNER JOin estoque e on p.id = e.id_produto
    ...
    async selectProduct(req, res) {
        ...
        CASE
          ...
          ELSE 0 
        END AS discount,
        e.id as id_estoque, 0 as qtd, p.principio, e.fabricante, p.registroms, p.id_principio, p.classe_terapeutica
    FROM produtos p
    inner JOIN estoque e ON p.id = e.id_produto
