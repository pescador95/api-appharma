      ...
      try {
         ...
         cart.map((i, k) => {
            ...
            for (let j = 0; j < i.qtd; j++) {
               Venda.create({ status:'Pendente', codigo_venda: uuid, id_user: userId, id_produto: id, codigo_barras, nome, valor_liquido: preco_vigente, cpf, created_at: now, updated_at: now, data_venda: now, tipo_venda, levar_pinpad, troco_para, tipo_entrega, id_endereco })
            }
         })
